...
if page == "ホーム":
    ...
    with col2:
        st.image("jisan.webp", caption="山中筋肉", use_container_width=True)
...
# トレーニング履歴ページ
    ...
    try:
        # トレーニング履歴を読み込む
        with open("workout_log.txt", "r", encoding="utf-8") as f:
            logs = f.readlines()
        ...
        if logs:
            ...
            for i, log in enumerate(logs, start=1):
                ...
                if col2.button(f"削除 {i}", key=f"delete_{i}"):
                    logs.pop(i - 1)  # 履歴を削除
                    # 新しい履歴を書き戻す
                    with open("workout_log.txt", "w", encoding="utf-8") as f:
                        f.writelines(logs)
